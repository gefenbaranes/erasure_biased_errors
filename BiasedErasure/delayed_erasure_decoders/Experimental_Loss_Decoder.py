...
def Loss_MLE_Decoder_Experiment(config: Config, measurement_events: np.ndarray, detection_events_signs: np.ndarray):
        """
        ...
        We use the MLE approximate decoding - for each potential loss event we create a DEM and connect all to generate the final DEM. 
        Input: config object with meta parameters and distances, num shots, and experimental data.
        Output: final DEM, corrections, num errors.
        """
        # Initialize the Simulator with the Config object
        simulator = Simulator(config=config)
        num_shots = measurement_events.shape[0]
        
        if not logical_gaps:
                predictions, observable_flips, dems_list = simulator.count_logical_errors_experiment(num_shots = num_shots, 
                ...
                predictions, log_probabilities, observable_flips, dems_list = simulator.count_logical_errors_experiment(num_shots = num_shots,
                                                        measurement_events = measurement_events, 
                                                        detection_events_signs=detection_events_signs, 
                                                        logical_gap = True)
